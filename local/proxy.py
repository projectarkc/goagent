...
sys.path = [(os.path.dirname(__file__) or '.') + '/packages.egg/noarch'] + sys.path + \
    [(os.path.dirname(__file__) or '.') + '/packages.egg/' +
    ...
    def __init__(self, *args, **kwargs):
        ...
        if hasattr(sys.stderr, 'isatty') and sys.stderr.isatty():
            if os.name == 'nt':
                ...
                self.__set_error_color = lambda: SetConsoleTextAttribute(
                    GetStdHandle(-11), 0x04)
                self.__set_warning_color = lambda: SetConsoleTextAttribute(
                    GetStdHandle(-11), 0x06)
                self.__set_debug_color = lambda: SetConsoleTextAttribute(
                    GetStdHandle(-11), 0x002)
                self.__reset_color = lambda: SetConsoleTextAttribute(
                    GetStdHandle(-11), 0x07)
            elif os.name == 'posix':
...
class RangeFetch(object):
    ...
    def fetch(self):
        ...
        logging.info(
            '>>>>>>>>>>>>>>> RangeFetch started(%r) %d-%d', self.url, start, end)
    ...
    def __fetchlet(self, range_queue, data_queue, range_delay_size):
        ...
        while 1:
            try:
                ...
                try:
                    ...
                    if not response:
                        ...
                        if self._last_app_status.get(fetchserver, 200) >= 500:
                            time.sleep(5)
                        response = self.plugin.fetch(self.handler, self.handler.command, self.url, headers, self.handler.body,
                                                     timeout=self.handler.net2.connect_timeout, fetchserver=fetchserver, **self.kwargs)
                except Queue.Empty:
                    continue
                except Exception as e:
                    logging.warning(
                ...
                if not response:
                    logging.warning(
                        'RangeFetch %s return %r', headers['Range'], response)
                    range_queue.put((start, end, None))
                    continue
                if fetchserver:
                    self._last_app_status[fetchserver] = response.app_status
                if response.app_status != 200:
                    logging.warning('Range Fetch "%s %s" %s return %s', self.handler.command, self.url, headers[
                                    'Range'], response.app_status)
                    response.close()
                    range_queue.put((start, end, None))
                    continue
                ...
                if 200 <= response.status < 300:
                    ...
                    if not content_range:
                        logging.warning('RangeFetch "%s %s" return Content-Range=%r: response headers=%r, retry %s-%s',
                                        self.handler.command, self.url, content_range, response.getheaders(), start, end)
                        response.close()
                        range_queue.put((start, end, None))
                        continue
                    content_length = int(
                        response.getheader('Content-Length', 0))
                    logging.info('>>>>>>>>>>>>>>> [thread %s] %s %s', threading.currentThread(
                    ...
                    while 1:
                        try:
                            ...
                            logging.warning(
                                'RangeFetch "%s %s" %s failed: %s', self.handler.command, self.url, headers['Range'], e)
                            break
                    if start < end + 1:
                        logging.warning(
                            'RangeFetch "%s %s" retry %s-%s', self.handler.command, self.url, start, end)
                        response.close()
                        range_queue.put((start, end, None))
                        continue
                    logging.info(
                    ...
                    logging.error(
                        'RangeFetch %r return %s', self.url, response.status)
                    response.close()
                    range_queue.put((start, end, None))
                    continue
...
class GAEFetchPlugin(BaseFetchPlugin):
    ...
    def handle(self, handler, **kwargs):
        ...
        body = handler.body
        if handler.path[0] == '/':
            url = '%s://%s%s' % (handler.scheme,
                                 handler.headers['Host'], handler.path)
        ...
        for i in xrange(self.max_retry):
            try:
                ...
                response = self.fetch(
                    handler, method, url, headers, body, handler.net2.connect_timeout)
                if response.app_status < 500:
                    ...
                    if response.app_status == 503:
                        ...
                        if len(self.appids) > 1:
                            ...
                            logging.info(
                                'gae over qouta, switch next appid=%r', self.appids[0])
                    elif i < self.max_retry - 1 and len(self.appids) > 1:
                        self.appids.append(self.appids.pop(0))
                        logging.info(
                            'URLFETCH return %d, trying next appid=%r', response.app_status, self.appids[0])
                    response.close()
            except Exception as e:
                errors.append(e)
                logging.info('GAE "%s %s" appid=%r %r, retry...',
                             handler.command, handler.path, self.appids[0], e)
        if len(errors) == self.max_retry:
            if response and response.app_status >= 500:
                ...
                content = message_html('502 URLFetch failed', 'Local URLFetch %r failed' % handler.path, '<br>'.join(
                    str(x).decode(locale.getpreferredencoding()) for x in errors))
            return handler.handler_plugins['mock'].handle(handler, status, headers, content)
        logging.info('%s "GAE %s %s %s" %s %s', handler.address_string(), handler.command, handler.path,
                     handler.protocol_version, response.status, response.getheader('Content-Length', '-'))
        try:
            if response.status == 206 and not rescue_bytes:
                fetchservers = ['%s://%s.appspot.com%s' %
                                (self.mode, x, self.path) for x in self.appids]
            ...
            while True:
                ...
                if data is None:
                    logging.warning(
                        'GAE response.read(%r) %r timeout', bufsize, url)
    ...
    def fetch(self, handler, method, url, headers, body, timeout, **kwargs):
        ...
        payload += ''.join('%s: %s\r\n' % (k, v)
                           for k, v in headers.items() if k not in handler.net2.skip_headers)
        payload += ''.join('X-URLFETCH-%s: %s\r\n' % (k, v)
                           for k, v in kwargs.items() if v)
        # prepare GAE request
        request_method = 'POST'
        fetchserver_index = random.randint(
            0, len(self.appids) - 1) if 'Range' in headers else 0
        fetchserver = kwargs.get(
            'fetchserver') or '%s://%s.appspot.com%s' % (self.mode, self.appids[fetchserver_index], self.path)
        request_headers = {}
        if common.GAE_OBFUSCATE:
            request_method = 'GET'
            fetchserver += 'ps/%d%s.gif' % (
                int(time.time() * 1000), random.random())
            request_headers[
                'X-URLFETCH-PS1'] = base64.b64encode(deflate(payload)).strip()
            if body:
                request_headers[
                    'X-URLFETCH-PS2'] = base64.b64encode(deflate(body)).strip()
                body = ''
            if common.GAE_PAGESPEED:
                fetchserver = re.sub(
                    r'^(\w+://)', r'\g<1>1-ps.googleusercontent.com/h/', fetchserver)
        else:
        ...
        cache_key = '%s:%d' % (handler.net2.host_postfix_map.get(
            '.appspot.com', ''), 443 if common.GAE_MODE == 'https' else 80)
        ...
        if 'rc4' in request_headers.get('X-URLFETCH-Options', ''):
            response.fp = CipherFileObject(
                response.fp, RC4Cipher(kwargs['password']))
        data = response.read(4)
        if len(data) < 4:
            response.status = 502
            response.fp = io.BytesIO(
                b'connection aborted. too short leadbyte data=' + data)
...
class PHPFetchPlugin(BaseFetchPlugin):
    ...
    def handle(self, handler, **kwargs):
        ...
        body = '%s%s%s' % ((struct.pack('!h', len(payload)), payload, body))
        request_headers = {
            'Content-Length': len(body), 'Content-Type': 'application/octet-stream'}
        fetchserver_index = 0 if 'Range' not in headers else random.randint(
            0, len(self.fetchservers) - 1)
        fetchserver = '%s?%s' % (
            self.fetchservers[fetchserver_index], random.random())
        crlf = 0
        cache_key = '%s//:%s' % urlparse.urlsplit(fetchserver)[:2]
        try:
            response = handler.net2.create_http_request(
                'POST', fetchserver, request_headers, body, handler.net2.connect_timeout, crlf=crlf, cache_key=cache_key)
        except Exception as e:
...
class VPSServer(gevent.server.StreamServer):
    ...
    def handle(self, sock, addr):
        ...
        if netloc.rfind(':') <= netloc.rfind(']'):
            ...
            port = int(port)
        remote = self.net2.create_ssl_connection(
            host, port, 8, cache_key=netloc)
        request_data = '%s\r\nProxy-Authorization: Baisic %s\r\n%s' % (
            request_line, base64.b64encode('%s:%s' % (username, password)).strip(), header_data)
    ...
    def filter(self, handler):
        ...
        if handler.command == 'CONNECT':
            ...
            if 'php' in handler.handler_plugins:
                ...
                logging.warning(
                    '"%s %s" not supported by GAE, please enable PHP mode!', handler.command, handler.path)
                return 'direct', {}
...
class WithGAEFilter(BaseProxyHandlerFilter):
    ...
    def __init__(self, withgae_sites, withphp_sites):
        self.withgae_sites = set(
            x for x in withgae_sites if not x.startswith('.'))
        self.withgae_sites_postfix = tuple(
            x for x in withgae_sites if x.startswith('.'))
        self.withphp_sites = set(
            x for x in withphp_sites if not x.startswith('.'))
        self.withphp_sites_postfix = tuple(
            x for x in withphp_sites if x.startswith('.'))
    ...
    def filter(self, handler):
        ...
        if handler.host in self.withgae_sites or handler.host.endswith(self.withgae_sites_postfix):
            ...
            if 'php' not in handler.handler_plugins:
                logging.warning(
                    'handler=%s does not contains php plugin, fallback to gae plugin!', handler)
                plugin = 'gae'
            else:
                plugin = 'php'
        if plugin:
...
class GAEProxyHandler(SimpleProxyHandler):
    ...
    def first_run(self):
        ...
        if not common.PROXY_ENABLE:
            logging.info(
                'resolve common.IPLIST_ALIAS names=%s to iplist', list(common.IPLIST_ALIAS))
            common.resolve_iplist()
        random.shuffle(common.GAE_APPIDS)
        self.__class__.handler_plugins['gae'] = GAEFetchPlugin(common.GAE_APPIDS, common.GAE_PASSWORD, common.GAE_PATH, common.GAE_MODE,
                                                               common.GAE_CACHESOCK, common.GAE_KEEPALIVE, common.GAE_OBFUSCATE, common.GAE_PAGESPEED, common.GAE_VALIDATE, common.GAE_OPTIONS)
        if not common.PROXY_ENABLE:
            net2 = AdvancedNet2(window=common.GAE_WINDOW, ssl_version=common.GAE_SSLVERSION,
                                dns_servers=common.DNS_SERVERS, dns_blacklist=common.DNS_BLACKLIST)
    ...
    def extend_iplist(self, iplist_name):
        hosts = [x for x in common.CONFIG.get('iplist', iplist_name).split(
            '|') if not re.match(r'^\d+\.\d+\.\d+\.\d+$', x) and ':' not in x]
            ...
            for dnslib_resolve in (dnslib_resolve_over_udp, dnslib_resolve_over_tcp):
                try:
                    ...
                    iplist = dnslib_record2iplist(
                        dnslib_resolve(host, [dnsserver], timeout=4, blacklist=common.DNS_BLACKLIST))
                    queue.put((host, dnsserver, iplist))
                except (socket.error, OSError) as e:
                    logging.info(
                        '%s remote host=%r failed: %s', str(dnslib_resolve).split()[1], host, e)
                    time.sleep(1)
        result_queue = Queue.Queue()
        pool = __import__('gevent.pool', fromlist=['.']).Pool(
            8) if sys.modules.get('gevent') else None
        for host in hosts:
            for dnsserver in common.DNS_SERVERS:
                logging.debug(
                    'remote resolve host=%r from dnsserver=%r', host, dnsserver)
                if pool:
                    pool.spawn(
                        do_remote_resolve, host, dnsserver, result_queue)
                else:
        ...
        for _ in xrange(len(common.DNS_SERVERS) * len(hosts) * 2):
            try:
                ...
                logging.debug(
                    '%r remote host=%r return %s', dnsserver, host, iplist)
                if '.google' in host:
                    if common.GAE_IPV6:
                ...
                break
        logging.info('extend_iplist finished, added %s', len(
            set(self.net2.iplist_alias[iplist_name]) - set(new_iplist)))
...
class PHPProxyHandler(SimpleProxyHandler):
    ...
    def first_run(self):
        ...
        if not common.PROXY_ENABLE:
            ...
            net2 = AdvancedNet2(
                window=4, ssl_version='TLSv1', dns_servers=common.DNS_SERVERS, dns_blacklist=common.DNS_BLACKLIST)
...
class PacUtil(object):
    ...
    def update_pacfile(filename):
        ...
        try:
            ...
            content = re.sub(
                r'''blackhole\s*=\s*['"]PROXY [\.\w:]+['"]''', 'blackhole = \'PROXY %s\'' % blackhole, content)
            content = re.sub(
                r'''autoproxy\s*=\s*['"]PROXY [\.\w:]+['"]''', 'autoproxy = \'PROXY %s\'' % autoproxy, content)
            content = re.sub(
                r'''defaultproxy\s*=\s*['"](DIRECT|PROXY [\.\w:]+)['"]''', 'defaultproxy = \'%s\'' % default, content)
            content = re.sub(
                r'''host\s*==\s*['"][\.\w:]+['"]\s*\|\|\s*isPlainHostName''', 'host == \'%s\' || isPlainHostName' % listen_ip, content)
        ...
        try:
            if common.PAC_ADBLOCK:
                ...
                logging.info(
                    'try download %r to update_pacfile(%r)', common.PAC_ADBLOCK, filename)
                adblock_content = PacUtil.urlread(
                    common.PAC_ADBLOCK, autoproxy)
                logging.info(
                    '%r downloaded, try convert it with adblock2pac', common.PAC_ADBLOCK)
        ...
        try:
            ...
            for url in common.PAC_GFWLIST.split('|'):
                logging.info(
                    'try download %r to update_pacfile(%r)', url, filename)
            ...
            logging.info(
                '%r downloaded, try convert it with autoproxy2pac_lite', common.PAC_GFWLIST)
            if 'gevent' in sys.modules and time.sleep is getattr(sys.modules['gevent'], 'sleep', None) and hasattr(gevent.get_hub(), 'threadpool'):
                jsrule = gevent.get_hub().threadpool.apply_e(Exception, PacUtil.autoproxy2pac_lite,
                                                             (autoproxy_content, 'FindProxyForURLByAutoProxy', autoproxy, default))
            else:
                jsrule = PacUtil.autoproxy2pac_lite(
                    autoproxy_content, 'FindProxyForURLByAutoProxy', autoproxy, default)
            content += '\r\n' + jsrule + '\r\n'
        ...
        for line in content.splitlines()[1:]:
            if line and not line.startswith("!"):
                ...
                if line.startswith('/') and line.endswith('/'):
                    jsLine = 'if (/%s/i.test(url)) return "%s";' % (
                        line[1:-1], return_proxy)
                    ...
                    jsLine = 'if (url.indexOf("%s") == 0) return "%s";' % (
                        line[1:], return_proxy)
                elif '*' in line:
                    jsLine = 'if (shExpMatch(url, "*%s*")) return "%s";' % (
                        line.strip('*'), return_proxy)
                elif '/' not in line:
                    jsLine = 'if (host.indexOf("%s") >= 0) return "%s";' % (
                        line, return_proxy)
                else:
                    jsLine = 'if (url.indexOf("%s") >= 0) return "%s";' % (
                        line, return_proxy)
    ...
    def autoproxy2pac_lite(content, func_name='FindProxyForURLByAutoProxy', proxy='127.0.0.1:8087', default='DIRECT', indent=4):
                ...
                try:
                    if line.startswith('/') and line.endswith('/'):
                        ...
                        if line.startswith('^https?:\\/\\/[^\\/]+') and re.match(r'^(\w|\\\-|\\\.)+$', line[18:]):
                            ...
                            logging.warning(
                                'unsupport gfwlist regex: %r', line)
                    elif line.startswith('||'):
                        domain = line[2:].lstrip('*').rstrip('/')
                    elif line.startswith('|'):
                        domain = urlparse.urlsplit(
                            line[1:]).hostname.lstrip('*')
                    ...
                    logging.warning(
                        'error when process gfwlist rule: %r %s', line, e)
                if '*' in domain:
        ...
        proxy_domain_list = sorted(set(x.lstrip('.')
                                       for x in proxy_domain_set))
        autoproxy_host = ',\r\n'.join(
            '%s"%s": 1' % (' ' * indent, x) for x in proxy_domain_list)
        template = '''\
                    var autoproxy_host = {
                    %(autoproxy_host)s
                    };
        ...
        template = re.sub(r'(?m)^\s{%d}' % min(
            len(re.search(r' +', x).group()) for x in template.splitlines()), '', template)
    ...
    def adblock2pac(content, func_name='FindProxyForURLByAdblock', proxy='127.0.0.1:8086', default='DIRECT', admode=1, indent=4):
        ...
        for line in content.splitlines()[1:]:
            ...
            if use_start and use_end:
                ...
                if '*' in line:
                    if use_postfix:
                        conditions['shExpMatch'] += ['*%s*%s' %
                                                     (line, x) for x in use_postfix]
                    else:
                        conditions['shExpMatch'] += ['*%s*' % line]
                else:
        ...
        template = re.sub(r'(?m)^\s{%d}' % min(len(re.search(
            r' +', x).group()) for x in templates[admode].splitlines()), '', templates[admode])
...
class PacFileFilter(BaseProxyHandlerFilter):
    ...
    def filter(self, handler):
        ...
        pacfile = os.path.join(
            os.path.dirname(os.path.abspath(__file__)), common.PAC_FILE)
        ...
        if handler.command == 'GET' and urlparts.path.lstrip('/') == common.PAC_FILE:
            ...
            if time.time() - os.path.getmtime(pacfile) > common.PAC_EXPIRED:
                ...
                if uptime and uptime > 1800:
                    thread.start_new_thread(lambda: os.utime(
                        pacfile, (time.time(), time.time())) or PacUtil.update_pacfile(pacfile), tuple())
            with open(pacfile, 'rb') as fp:
                ...
                if 'gzip' in handler.headers.get('Accept-Encoding', ''):
                    ...
                    dataio.write(struct.pack(
                        '<LL', zlib.crc32(content) & 0xFFFFFFFFL, len(content) & 0xFFFFFFFFL))
...
class Common(object):
    ...
    def __init__(self):
        ...
        self.CONFIG_FILENAME = os.path.splitext(
            os.path.abspath(__file__))[0] + '.ini'
        self.CONFIG_USER_FILENAME = re.sub(
            r'\.ini$', '.user.ini', self.CONFIG_FILENAME)
        self.CONFIG.read([self.CONFIG_FILENAME, self.CONFIG_USER_FILENAME])

        for key, value in os.environ.items():
            m = re.match(r'^%s([A-Z]+)_([A-Z\_\-]+)$' %
                         self.ENV_CONFIG_PREFIX, key)
            if m:
                self.CONFIG.set(m.group(1).lower(), m.group(2).lower(), value)

        self.LISTEN_IP = self.CONFIG.get('listen', 'ip')
        self.LISTEN_PORT = self.CONFIG.getint('listen', 'port')
        self.LISTEN_USERNAME = self.CONFIG.get(
            'listen', 'username') if self.CONFIG.has_option('listen', 'username') else ''
        self.LISTEN_PASSWORD = self.CONFIG.get(
            'listen', 'password') if self.CONFIG.has_option('listen', 'password') else ''
        ...
        self.GAE_TRANSPORT = self.CONFIG.getint(
            'gae', 'transport') if self.CONFIG.has_option('gae', 'transport') else 0
        ...
        self.GAE_PAGESPEED = self.CONFIG.getint(
            'gae', 'pagespeed') if self.CONFIG.has_option('gae', 'pagespeed') else 0
        ...
        if self.GAE_IPV6:
            ...
            try:
                ...
                logging.info(
                    'use ipv6 interface %s for gae', sock.getsockname()[0])
            except Exception as e:
        ...
        if 'USERDNSDOMAIN' in os.environ and re.match(r'^\w+\.\w+$', os.environ['USERDNSDOMAIN']):
            self.CONFIG.set(
                'profile', '.' + os.environ['USERDNSDOMAIN'], 'direct')
        ...
        self.IPLIST_PREDEFINED = [x for x in sum(
            self.IPLIST_ALIAS.values(), []) if re.match(r'^\d+\.\d+\.\d+\.\d+$', x) or ':' in x]
        ...
        self.PHP_PASSWORD = self.CONFIG.get(
            'php', 'password') if self.CONFIG.has_option('php', 'password') else ''
        self.PHP_CRLF = self.CONFIG.getint(
            'php', 'crlf') if self.CONFIG.has_option('php', 'crlf') else 1
        self.PHP_VALIDATE = self.CONFIG.getint(
            'php', 'validate') if self.CONFIG.has_option('php', 'validate') else 0
        self.PHP_KEEPALIVE = self.CONFIG.getint('php', 'keepalive')
        self.PHP_FETCHSERVER = self.CONFIG.get('php', 'fetchserver')
        self.PHP_HOSTS = self.CONFIG.get('php', 'hosts').split(
            '|') if self.CONFIG.get('php', 'hosts') else []
        ...
        self.PROXY_AUTODETECT = self.CONFIG.getint(
            'proxy', 'autodetect') if self.CONFIG.has_option('proxy', 'autodetect') else 0
        ...
        if not self.PROXY_ENABLE and self.PROXY_AUTODETECT:
            ...
            if system_proxy and self.LISTEN_IP not in system_proxy:
                _, username, password, address = ProxyUtil.parse_proxy(
                    system_proxy)
        ...
        self.AUTORANGE_ENDSWITH = tuple(
            self.CONFIG.get('autorange', 'endswith').split('|'))
        self.AUTORANGE_NOENDSWITH = tuple(
            self.CONFIG.get('autorange', 'noendswith').split('|'))
        self.AUTORANGE_MAXSIZE = self.CONFIG.getint('autorange', 'maxsize')
        self.AUTORANGE_WAITSIZE = self.CONFIG.getint('autorange', 'waitsize')
        self.AUTORANGE_BUFSIZE = self.CONFIG.getint('autorange', 'bufsize')
        self.AUTORANGE_THREADS = self.CONFIG.getint('autorange', 'threads')

        self.FETCHMAX_LOCAL = self.CONFIG.getint(
            'fetchmax', 'local') if self.CONFIG.get('fetchmax', 'local') else 3
        self.FETCHMAX_SERVER = self.CONFIG.get('fetchmax', 'server')

        self.DNS_ENABLE = self.CONFIG.getint('dns', 'enable')
        self.DNS_LISTEN = self.CONFIG.get('dns', 'listen')
        self.DNS_SERVERS = self.HTTP_DNS or self.CONFIG.get(
            'dns', 'servers').split('|')
        self.DNS_BLACKLIST = set(
            self.CONFIG.get('dns', 'blacklist').split('|'))
        self.DNS_TCPOVER = tuple(self.CONFIG.get('dns', 'tcpover').split(
            '|')) if self.CONFIG.get('dns', 'tcpover').strip() else tuple()
        if self.GAE_IPV6:
    ...
    def resolve_iplist(self):
        ...
        def do_local_resolve(host, queue):
            ...
            for _ in xrange(3):
                try:
                    ...
                    iplist = [x[-1][0]
                              for x in socket.getaddrinfo(host, 80, family)]
                    queue.put((host, iplist))
                except (socket.error, OSError) as e:
                    logging.warning(
                        'socket.getaddrinfo host=%r failed: %s', host, e)
                    time.sleep(0.1)
        google_blacklist = ['216.239.32.20'] + list(self.DNS_BLACKLIST)
        google_blacklist_prefix = tuple(
            x for x in self.DNS_BLACKLIST if x.endswith('.'))
        for name, need_resolve_hosts in list(self.IPLIST_ALIAS.items()):
            if all(re.match(r'\d+\.\d+\.\d+\.\d+', x) or ':' in x for x in need_resolve_hosts):
                continue
            need_resolve_remote = [
                x for x in need_resolve_hosts if ':' not in x and not re.match(r'\d+\.\d+\.\d+\.\d+', x)]
            resolved_iplist = [
                x for x in need_resolve_hosts if x not in need_resolve_remote]
            result_queue = Queue.Queue()
            ...
            if name.startswith('google_'):
                ...
                resolved_iplist = [
                    x for x in resolved_iplist if not x.startswith(google_blacklist_prefix)]
            if len(resolved_iplist) == 0 and name in ('google_hk', 'google_cn') and not self.GAE_IPV6:
                logging.error(
                    'resolve %s host return empty! please retry!', name)
                sys.exit(-1)
            logging.info(
                'resolve name=%s host to iplist=%r', name, resolved_iplist)
            common.IPLIST_ALIAS[name] = resolved_iplist
        if self.IPLIST_ALIAS.get('google_cn', []):
            try:
                for _ in xrange(4):
                    socket.create_connection(
                        (random.choice(self.IPLIST_ALIAS['google_cn']), 80), timeout=2).close()
            except socket.error:
                self.IPLIST_ALIAS['google_cn'] = []
        if len(self.IPLIST_ALIAS.get('google_cn', [])) < 4 and self.IPLIST_ALIAS.get('google_hk', []):
            logging.warning(
                'google_cn resolved too short iplist=%s, switch to google_hk', self.IPLIST_ALIAS.get('google_cn', []))
    ...
    def summary(self):
        ...
        info += 'Uvent Version      : %s (pyuv/%s libuv/%s)\n' % (__import__('uvent').__version__, __import__(
            'pyuv').__version__, __import__('pyuv').LIBUV_VERSION) if all(x in sys.modules for x in ('pyuv', 'uvent')) else ''
        ...
        info += 'GAE Mode           : %s\n' % ('%s (%s)' % (
            self.GAE_MODE, self.GAE_SSLVERSION) if common.GAE_MODE == 'https' else self.GAE_MODE)
...
def pre_start():
    if sys.platform == 'cygwin':
        logging.info(
            'cygwin is not officially supported, please continue at your own risk :)')
        # sys.exit(-1)
    elif os.name == 'posix':
        try:
            import resource
        ...
        if common.LOVE_ENABLE and random.randint(1, 100) <= 5:
            ...
            ctypes.windll.kernel32.GetConsoleTitleW(
                ctypes.byref(title), len(title) - 1)
    ...
    if os.path.isfile('/proc/cpuinfo'):
        with open('/proc/cpuinfo', 'rb') as fp:
            m = re.search(
                r'(?im)(BogoMIPS|cpu MHz)\s+:\s+([\d\.]+)', fp.read())
    ...
    if common.GAE_ENABLE and common.GAE_APPIDS[0] == 'goagent':
        logging.warning(
            'please edit %s to add your appid to [gae] !', common.CONFIG_FILENAME)
    if common.GAE_ENABLE and common.GAE_MODE == 'http' and common.GAE_PASSWORD == '':
        logging.warning(
            'to enable http mode, you should set %r [gae]password = <your_pass> and [gae]options = rc4', common.CONFIG_FILENAME)
    ...
    if common.PAC_ENABLE:
        ...
        spawn_later(
            600, urllib2.build_opener(urllib2.ProxyHandler({})).open, url)
    if not common.DNS_ENABLE:
        ...
        for dnsservers_ref in (common.HTTP_DNS, common.DNS_SERVERS):
            any(dnsservers_ref.insert(0, x)
                for x in [y for y in get_dnsserver_list() if y not in dnsservers_ref])
    ...
    if True:
        GAEProxyHandler.handler_filters.insert(0, AutoRangeFilter(
            common.AUTORANGE_HOSTS, common.AUTORANGE_ENDSWITH, common.AUTORANGE_NOENDSWITH, common.AUTORANGE_MAXSIZE))
    if common.GAE_REGIONS:
        GAEProxyHandler.handler_filters.insert(
            0, DirectRegionFilter(common.GAE_REGIONS))
    if common.CRLF_SITES:
        GAEProxyHandler.handler_filters.insert(
            0, CRLFSitesFilter(common.CRLF_SITES, common.NOCRLF_SITES))
    if common.URLREWRITE_MAP:
        GAEProxyHandler.handler_filters.insert(0, URLRewriteFilter(
            common.URLREWRITE_MAP, common.FORCEHTTPS_SITES, common.NOFORCEHTTPS_SITES))
    if common.FAKEHTTPS_SITES:
        GAEProxyHandler.handler_filters.insert(
            0, FakeHttpsFilter(common.FAKEHTTPS_SITES, common.NOFAKEHTTPS_SITES))
    if common.FORCEHTTPS_SITES:
        GAEProxyHandler.handler_filters.insert(
            0, ForceHttpsFilter(common.FORCEHTTPS_SITES, common.NOFORCEHTTPS_SITES))
    if common.WITHGAE_SITES or common.WITHPHP_SITES:
        GAEProxyHandler.handler_filters.insert(
            0, WithGAEFilter(common.WITHGAE_SITES, common.WITHPHP_SITES))
    if common.USERAGENT_ENABLE:
        GAEProxyHandler.handler_filters.insert(
            0, UserAgentFilter(common.USERAGENT_STRING))
    if common.LISTEN_USERNAME:
        GAEProxyHandler.handler_filters.insert(
            0, AuthFilter(common.LISTEN_USERNAME, common.LISTEN_PASSWORD))
...
def main():
    ...
    logging.basicConfig(level=logging.DEBUG if common.LISTEN_DEBUGINFO else logging.INFO,
                        format='%(levelname)s - %(asctime)s %(message)s', datefmt='[%b %d %H:%M:%S]')
    pre_start()
    sys.stderr.write(common.summary())

    if common.PAC_ENABLE:
        thread.start_new_thread(LocalProxyServer(
            (common.PAC_IP, common.PAC_PORT), PACProxyHandler).serve_forever, tuple())
    ...
    if common.DNS_ENABLE:
        try:
            ...
            dns_server = DNSServer((host, int(port)), dns_servers=common.DNS_SERVERS,
                                   dns_blacklist=common.DNS_BLACKLIST, dns_tcpover=common.DNS_TCPOVER)
            thread.start_new_thread(dns_server.serve_forever, tuple())
        except ImportError:
            logging.exception(
                'GoAgent DNSServer requires dnslib and gevent 1.0')
            sys.exit(-1)
    ...
    if common.PROXY_ENABLE:
        if common.GAE_ENABLE:
            GAEProxyHandler.net2 = ProxyNet2(
                common.PROXY_HOST, common.PROXY_PORT, common.PROXY_USERNAME, common.PROXY_PASSWROD)
        if common.PHP_ENABLE:
            PHPProxyHandler.net2 = ProxyNet2(
                common.PROXY_HOST, common.PROXY_PORT, common.PROXY_USERNAME, common.PROXY_PASSWROD)
        if common.VPS_ENABLE:
            VPSServer.net2 = ProxyNet2(
                common.PROXY_HOST, common.PROXY_PORT, common.PROXY_USERNAME, common.PROXY_PASSWROD)

    php_server = None
    if common.PHP_ENABLE:
        host, port = common.PHP_LISTEN.split(':')
        PHPProxyHandler.handler_plugins['php'] = PHPFetchPlugin(
            common.PHP_FETCHSERVER, common.PHP_PASSWORD, common.PHP_VALIDATE)
    ...
    if common.VPS_ENABLE:
        ...
        VPSServer.net2 = AdvancedNet2(
            window=2, ssl_version='SSLv23', dns_servers=common.DNS_SERVERS, dns_blacklist=common.DNS_BLACKLIST)
        ...
        VPSServer.net2.openssl_context.set_cipher_list(
            'RC4-MD5:RC4-SHA:!aNULL:!eNULL')
        vps_server = VPSServer(
            (host, int(port)), backlog=1024, fetchservers=common.VPS_FETCHSERVER.split('|'))
        thread.start_new_thread(vps_server.serve_forever, tuple())

    if common.GAE_ENABLE:
        if common.PHP_ENABLE:
            GAEProxyHandler.handler_plugins[
                'php'] = php_server.RequestHandlerClass.handler_plugins['php']
        if os.name == 'nt':
            GAEProxyHandler.handler_plugins['strip'] = StripPluginEx()
        gae_server = LocalProxyServer(
            (common.LISTEN_IP, common.LISTEN_PORT), GAEProxyHandler)
